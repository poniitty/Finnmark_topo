...
vars <- unique(unlist(lapply(list.files("tpi1000/", pattern = "tif$"), function(x) str_split(x, "_")[[1]][1])))

for(i in vars){
  print(i)
  # i <- "tpi1000"
  f <- list.files("tpi1000", pattern = paste0(i,"_"), full.names = T)
  ...
  for(ii in grid_cells$id){
    ...
    for(iii in 1:length(f2)) { 
      rast.list[iii] <- rast(f2[iii])
      crs(rast.list[iii][[1]]) <- as.character(crs(aoi))
    }
